...
vec3 spring_force(vec3 const& p_i, vec3 const& p_j, float L_0, float K){
	float L = sqrt((p_i[0]-p_j[0])*(p_i[0] - p_j[0]) + (p_i[1] - p_j[1])*(p_i[1] - p_j[1]) + (p_i[2] - p_j[2])* (p_i[2] - p_j[2]));
	vec3 u = (p_i-p_j)/L;
	vec3 F = -K * (L - L0) * u;
    return F;
}
...
void display_frame()
{
    ...
    vec3 const fB = fB_spring + f_weight + fB_damping;

	vec3 const fC_spring = spring_force(pC, pB, L0, K);
	vec3 const fC_damping = -mu * vC;
	vec3 const fC = fC_spring + f_weight + fC_damping;
	...
	// particle pc
	sphere.transform.translate = pC;
	sphere.shading.color = { 0,1,0 };
	draw(sphere, scene);

	display_segment(pA, pB);
	display_segment(pB, pC);
